...
const sidebarSections = [
  { id: "account", label: "My account", active: true },
];
...
export default function SettingsPage() {
  ...
  const validateWebsite = (url: string): { isValid: boolean; error?: string } => {
    ...
    try {
      ...
      if (!['http:', 'https:'].includes(urlObj.protocol)) {
        return { isValid: false, error: "Website must be a valid HTTP/HTTPS URL" };
      }
      
      const hostname = urlObj.hostname;
      const tldMatch = hostname.match(/\.([a-z]{2,})$/i);
      if (!tldMatch) {
        return { isValid: false, error: "Website must have a valid top-level domain" };
      }
  ...
  const handleSave = async (field: string) => {
    ...
    try {
      const websiteForValidation = field === "website" && tempValue
        ? (tempValue.startsWith('http') ? tempValue : `https://${tempValue}`)
        : (field === "website" ? "" : profileData.website);

      const updateData = {
        name: field === "name" ? tempValue.trim() : profileData.name,
        location: field === "location" ? tempValue.trim() : profileData.location,
        website: field === "website" ? websiteForValidation : profileData.website
      };
            ...
            <div className="space-y-4">
              ...
              <div className="border border-border/40 rounded-lg p-4 sm:p-6">
                ...
                {accounts.length > 0 ? (
                  ...
                  <p className="text-muted-foreground text-sm">No connected providers found.</p>
                )}
              </div>
